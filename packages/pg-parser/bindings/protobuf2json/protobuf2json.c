/*
 * Adapted from https://github.com/Sannis/protobuf2json-c
 *
 * ---
 *
...
protobuf_c_message_descriptor_get_field_by_json_name(const ProtobufCMessageDescriptor *desc,
                                                     const char *json_name) {
  if (desc == NULL)
    return NULL;

  // Linear search through all fields since we can't use binary search on json_name
  for (unsigned i = 0; i < desc->n_fields; i++) {
    const ProtobufCFieldDescriptor *field = &desc->fields[i];
    if (strcmp(field->reserved2, json_name) == 0) {
      return field;
    }
  }
  ...
  *json_message = json_object();
  ...
  for (i = 0; i < protobuf_message->descriptor->n_fields; i++) {
    ...
    if (field_descriptor->label == PROTOBUF_C_LABEL_REQUIRED) {
      ...
      if (json_object_set_new(*json_message, field_descriptor->reserved2, json_value)) {
        SET_ERROR_STRING_AND_RETURN(
    ...
    } else if (field_descriptor->label == PROTOBUF_C_LABEL_REPEATED) {  // PROTOBUF_C_LABEL_REPEATED
      ...
      if (*protobuf_values_count) {
        ...
        if (json_object_set_new(*json_message, field_descriptor->reserved2, array)) {
          SET_ERROR_STRING_AND_RETURN(
              PROTOBUF2JSON_ERR_JANSSON_INTERNAL,
              "Error in json_object_set_new()");
        }
      ...
      SET_ERROR_STRING_AND_RETURN(
          PROTOBUF2JSON_ERR_UNSUPPORTED_FIELD_TYPE,
          "Unsupported field label %d for field '%s' in message '%s'",
          field_descriptor->label, field_descriptor->name, protobuf_message->descriptor->name);
    }
  }
...
static int json2protobuf_process_field(
  ...
  } else if (field_descriptor->type == PROTOBUF_C_TYPE_UINT32 || field_descriptor->type == PROTOBUF_C_TYPE_FIXED32) {
    if (!json_is_integer(json_value)) {
      SET_ERROR_STRING_AND_RETURN(
          PROTOBUF2JSON_ERR_IS_NOT_INTEGER,
          "JSON value is not an integer required for GPB %s",
  ...
  } else if (field_descriptor->type == PROTOBUF_C_TYPE_INT64 || field_descriptor->type == PROTOBUF_C_TYPE_SINT64 || field_descriptor->type == PROTOBUF_C_TYPE_SFIXED64) {
    if (!json_is_integer(json_value)) {
      SET_ERROR_STRING_AND_RETURN(
          PROTOBUF2JSON_ERR_IS_NOT_INTEGER,
          "JSON value is not an integer required for GPB %s",
  ...
  } else if (field_descriptor->type == PROTOBUF_C_TYPE_UINT64 || field_descriptor->type == PROTOBUF_C_TYPE_FIXED64) {
    if (!json_is_integer(json_value)) {
      SET_ERROR_STRING_AND_RETURN(
          PROTOBUF2JSON_ERR_IS_NOT_INTEGER,
          "JSON value is not an integer required for GPB %s",
...
static int json2protobuf_process_message(
  ...
  *protobuf_message = calloc(1, protobuf_message_descriptor->sizeof_message);
  ...
  json_object_foreach(json_object, json_key, json_object_value) {
    const ProtobufCFieldDescriptor *field_descriptor = protobuf_c_message_descriptor_get_field_by_json_name(protobuf_message_descriptor, json_key);
    ...
    if (field_descriptor->label == PROTOBUF_C_LABEL_REQUIRED) {
      ...
      // Proto2 optional fields have presence tracking
      if (field_descriptor->type == PROTOBUF_C_TYPE_MESSAGE || field_descriptor->type == PROTOBUF_C_TYPE_STRING) {
        // Messages and strings are considered set when their pointer is non-null
      } else {
        // Scalar optional fields use the quantifier for presence
    ...
    } else if (field_descriptor->label == PROTOBUF_C_LABEL_REPEATED) {
      ...
      *protobuf_values_count = json_array_size(json_object_value);
      ...
      SET_ERROR_STRING_AND_RETURN(
          PROTOBUF2JSON_ERR_UNSUPPORTED_FIELD_TYPE,
          "Unsupported field label %d for field '%s' in message '%s'",
          field_descriptor->label, field_descriptor->name, protobuf_message_descriptor->name);
    }
  }
